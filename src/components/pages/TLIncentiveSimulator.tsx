...
import { ChevronLeft, ChevronDown, ChevronUp, Info, Lock, Unlock, Zap, Lightbulb } from 'lucide-react';
import { 
...
interface TLIncentiveSimulatorProps {
  onClose: () => void;
}

export function TLIncentiveSimulator({ onClose }: TLIncentiveSimulatorProps) {
  ...
  const incentiveSummary = getIncentiveSummary('TL');

  // Current MTD values (used ONLY for Auto Projection - READ ONLY)
  const mtdValues = {
    stockIns: 65, // Team stock-ins MTD
    inspections: 400, // Team inspections MTD
    gmvTotal: 2000000, // Total GMV ₹20L
    gmvFirstDisbursement: 800000, // First disbursement GMV ₹8L
    onboarding: 20, // DCF dealers onboarded
    score: 78, // TL Score (0-100)
  };
  ...
  const [whatIfInputs, setWhatIfInputs] = useState({
    expectedStockIns: 110,
    expectedInspections: 700,
    expectedGMVTotal: 3500000,
    expectedGMVFirst: 1200000,
    expectedOnboarding: 35,
    score: 78,
  });
  ...
  const autoContext: IncentiveContext = {
    role: 'TL',
    siActualMTD: mtdValues.stockIns,
    siTarget,
  ...
  const whatIfContext: WhatIfSimulationInput = {
    role: 'TL',
    projectedSI: whatIfInputs.expectedStockIns,
  ...
  const projectedSI = isAutoMode ? autoResult.projected.siProjected : whatIfInputs.expectedStockIns;
  const achievement = isAutoMode ? autoResult.projected.siAchievementPercent : (whatIfInputs.expectedStockIns / siTarget) * 100;
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 text-white p-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1">
            <h2 className="text-lg">TL Incentives</h2>
            <div className="text-xs opacity-90">Team earnings potential</div>
          </div>
        </div>
        ...
        <div className="bg-white/10 rounded-lg p-1 flex gap-1">
          ...
          >
            <div className="flex items-center justify-center gap-1.5">
              {simulatorMode === 'auto' ? <Lock className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5 opacity-60" />}
        ...
        <div className="p-4 space-y-4">
          ...
          <div className={`rounded-xl p-5 ${
            ...
            <div className="text-xs text-gray-500 mb-3">
              {isAutoMode 
                ? `Based on ${mtdValues.stockIns} MTD SIs → projects to ${projectedSI}`
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="text-gray-600">Stock-Ins Achievement</span>
              ...
              <div className="text-xs text-gray-500">
                {projectedSI} / {siTarget} Stock-ins
              </div>
            </div>
              ...
              <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-xs text-amber-800">
                    <strong>Target not met:</strong> Team needs {siTarget - projectedSI} more stock-ins to unlock incentives
                  </div>
                </div>
              </div>
            )}
          ...
          {simulatorMode === 'whatif' && (
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-purple-900">Adjust your team inputs</span>
              </div>
              ...
              <div>
                ...
                <input
                  type="number"
                  value={whatIfInputs.expectedStockIns}
                  onChange={(e) => setWhatIfInputs({ ...whatIfInputs, expectedStockIns: Number(e.target.value) })}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-blue-900">
                  {isAutoMode ? 'Team Performance Insights' : 'Action Tips'}
                </span>
              </div>
              <div className="space-y-2">
                {topInsights.map((insight, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    ...
                    <span className="text-xs text-blue-800">{insight.message}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {expandedSections.breakup && (
              <div className="p-4 space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <div>
                    <div className="text-sm text-gray-900">Stock-Ins Incentive</div>
            ...
            {expandedSections.benchmarks && (
              <div className="p-4 space-y-3">
                <div className="text-xs text-gray-600 mb-3">
                  Understanding how your team incentive is calculated
                </div>

                {/* SI Slabs */}
                <div className="bg-gray-50 rounded-lg p-3">
                  <div className="text-xs text-gray-700 mb-2">Stock-Ins Slabs</div>
