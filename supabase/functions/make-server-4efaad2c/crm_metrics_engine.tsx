/**
 * CRM API — Metrics Engine (Deno/Supabase Edge Function)
 * Phase: 6D | Ported from: /backend_api/metricsEngine.ts
 *
 * Config-driven SQL executor:
 *   1. Reads sql_template from metric_definitions
 *   2. Replaces named :params with positional $N bindings
 *   3. Executes safely via parameterized query
 *
 * CRITICAL SAFETY: ALL values go through parameterized binding.
 * sql_template is treated as trusted (admin-authored in DB).
 */

import db from "./crm_db.tsx";
import type { DateRange, AuthContext } from "./crm_utils.tsx";
import { getEffectiveContext } from "./crm_utils.tsx";
...
// ---------------------------------------------------------------------------
// Metric Definition Cache (TTL: 5 minutes)
// ---------------------------------------------------------------------------
...
/**
 * Replace named :param tokens with positional $N params.
...
export async function executeMetric(
  ...
  try {
    ...
    return result ?? 0; // DRD §0: NULL → 0
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[MetricsEngine] Error executing ${metricKey}:`, message);
